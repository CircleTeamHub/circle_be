...
describe('ChatPushService', () => {
  ...
  it('counts a JPush device that accepted the push as delivered', async () => {
    // 极光没有逐设备回执,收下即 CONFIRMED(Expo 是 SENT)。只认 SENT 的话,
    // 每个极光收件人都会被当成失败,扇出日志天天报红。
    prisma.chatMember.findMany.mockResolvedValue([seat('u2')]);
    push.listActiveTokensForUsers.mockImplementation((userIds: string[]) =>
      Promise.resolve(
        new Map(
          userIds.map((userId) => [
    ...
    expect(push.sendMessages.mock.calls[0][0]).toEqual([
      expect.objectContaining({
        token: 'jpush-registration',
        provider: 'jpush',
      }),
