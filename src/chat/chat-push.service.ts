...
export class ChatPushService implements OnModuleDestroy {
  ...
  private async sendToRecipients(
    ...
    let tokensByUser: Map<string, ActivePushToken[]>;
    try {
    ...
    outcomes.forEach((outcome, index) => {
      // Expo 收下是 SENT(之后还有回执);极光没有逐设备回执,收下即 CONFIRMED。
      if (outcome?.status === 'SENT' || outcome?.status === 'CONFIRMED') {
        delivered.add(owners[index]);
      } else {
        firstError ??= outcome?.error ?? outcome?.status ?? 'unknown';
      }
    });
