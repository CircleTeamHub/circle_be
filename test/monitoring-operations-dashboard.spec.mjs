...
const DASHBOARD_PATH = join(
  ROOT,
  'monitoring/grafana/dashboards/circle-be-operations.json',
);
...
test('queries cover alerts, target health, alert delivery, probes, and host capacity', () => {
  const value = dashboard();
  const expressions = value.panels.flatMap((panel) => panel.targets.map(({ expr }) => expr));
  ...
  assert.match(joined, /ALERTS\{alertstate="firing"\}/);
...
test('optional Alloy delivery rates stay absent rather than looking healthy', () => {
  ...
  assert.deepEqual(
    alloyPanel.targets.map(({ legendFormat }) => legendFormat),
    ['retries', 'dropped entries'],
  );
  assert.doesNotMatch(
    alloyPanel.targets.map(({ expr }) => expr).join('\n'),
    /or\s+(?:on\s*\([^)]*\)\s*)?vector/,
  );
});

test('host panels describe node_exporter as part of the base stack', () => {
  const descriptions = dashboard().panels
    .filter(({ title }) => /Host memory|host filesystem/.test(title))
