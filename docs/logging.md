...
- `LOG_FILE_ON=true` (can independently disable files; cannot override master-off)
...
If a client sends a 1–128 character ASCII alphanumeric/underscore/hyphen
`x-request-id`, the backend reuses it. Clients must send random opaque IDs,
never credentials or personal information. Otherwise the
