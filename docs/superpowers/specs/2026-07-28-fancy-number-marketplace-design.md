...
4. 首购从购买事务的服务器时间开始计算，续费从当前到期时间开始计算；两者都按 UTC 一次性增加所购的 N 个自然月，并保留 UTC 时、分、秒和毫秒。月末落到目标月最后一天。例如 1 月 31 日购买 1 个月得到 2 月最后一天，购买 2 个月直接得到 3 月 31 日，而不是逐月计算成 3 月 28 日。
5. 仅支持手动续费。
...
`source` 至少包含 `ADMIN` 和 `LEGACY`。管理员录入使用 `ADMIN` 且记录操作者；历史回填使用 `LEGACY` 且 `createdByUserID=null`，不伪造系统管理员。
...
`expiresAt=null` 且 `permanentAt!=null` 表示永久租约。普通首购必须有 `restoreAccountId`；只有历史永久迁移允许它为空，因为该租约永不到期。超级会员新选永久靓号仍必须保留购买前账号，便于审计，但永久租约不会自动恢复。使用 PostgreSQL 局部唯一索引保证：
...
表中的 `idempotencyKey` 不是直接保存调用方请求头，而是服务端生成的全局作用域键：

- 用户购买/续费：`client:{userID}:{headerValue}`；
...
请求头必须包含非空 `Idempotency-Key`，并限制最大长度。客户端值只在当前用户范围内唯一，服务端加用户前缀后持久化。
...
3. 永久租约返回不可续费。
4. 到期租约先恢复，然后返回到期冲突。
5. 以当前到期时间调用现有 `addUtcCalendarMonths(base, months)` 一次性顺延 1–12 个月并保留 UTC 时间部分。例如 1 月 31 日加 2 个月得到 3 月 31 日。
...
- 有仍在有效期内的租约时拒绝；
- 有已经到期但待清理的租约时，在同一事务中先完成账号恢复和靓号释放，再继续修改已恢复的普通账号，不返回依赖处理器时机的临时冲突；
- 通过 `AccountIdentifierModule` 取得新值；
- 同时检查账号、邀请码、原账号保留和靓号库存占用；
- 在事务内更新注册表和 `User.accountId`；
- 清理不再承担任何用途的旧普通标识。

所有创建用户的入口也必须在同一事务中登记标识，不能只更新主要注册接口。当前代码中的 `AuthService.createRegisteredUser` 和 `UserService.create` 都必须接入；邀请码当前只在这两个创建流程生成且没有修改接口，未来若增加邀请码修改路径，也必须通过注册表事务。

## 会员目录兼容

- 所有等级的旧 `benefits.fancyNumberVoucher` 都返回 `null`，避免旧客户端继续显示可兑换券。
- 在 `benefits` 中新增向后兼容字段 `permanentFancyNumber: boolean`；仅超级会员为 `true`，会员详情据此显示“永久靓号”。
- 旧 `benefitGrants.standardFancyNumber` / `premiumFancyNumber` 字段继续返回历史 `issued` 状态，但 `available` 永远为 `false`，不再代表可兑换资格。
...
所有购买、续费、永久化和到期事务按同一顺序取得锁：用户 → 当前有效租约 → 靓号 → 按账号值字典序排列的账号标识 → 钱包。统一使用现有 `runSerializableTransaction`，最多重试 3 次 `P2034`；唯一约束冲突不盲目重试，而是读取权威状态后映射为幂等成功或业务冲突。
...
缓存失效和实时广播是尽力而为的事务后副作用，失败不得回滚已提交交易，并记录结构化警告。账号资料需要同步到外部系统时，在业务事务内写入现有 `UserProfileSyncOutbox`，由已有处理器可靠重试；本期不新增购买或到期通知，避免为未要求的通知语义扩展持久化流程。
...
9. 恢复流量。
10. 保留现有 `User.accountId` 唯一索引作为第二层防线。
...
- 自然月：月末、闰年、跨年。
- 自然月多月：1 月 31 日一次加 2 个月得到 3 月 31 日，并保留 UTC 时间部分；首购以服务器事务时间为基准，续费以旧到期时间为基准。
...
- 修改账号：有效租约期间拒绝；已到期待清理时先恢复再完成改号；普通流程正确迁移注册表占用。
